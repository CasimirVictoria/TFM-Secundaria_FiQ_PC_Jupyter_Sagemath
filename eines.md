...
Donat que Antigravity està basat en VSCode, permet emprar totes les extensions pròpies d'aquest ecosistema, incloent-hi l'extensió per a treballar amb documents de Quarto, eines com LTeX per a la comprovació de la gramàtica i ortografia emprant LanguageTool, i extensions de Zotero per treballar amb la bibliografia recopilada d'una manera senzilla i productiva.
...
De fet, estic creant, amb l'ajuda de la pròpia IA, un protocol MCP (Model Context Protocol) per a realitzar cerques en diferents bases de dades, basant-me en projectes com [https://biomcp.org/](https://biomcp.org/). Evidentment, és una versió més limitada, però adaptada de manera molt senzilla —gràcies a la IA— a les meues necessitats específiques. També crearé i empraré un MCP per a treballar des del mateix IDE amb SageMath, enviant el codi a una instància de JupyterLab corrent com a servei de *systemctl* al meu ordinador. Tot això sense eixir de l'entorn de treball.
