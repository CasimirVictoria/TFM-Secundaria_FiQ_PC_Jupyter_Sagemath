...
async def test():
    ...
    if resp.status_code != 201:
        print(f"Error starting kernel: {resp.text}")
        return
    ...
    try:
        ...
        if xsrf: ws_headers["Cookie"] = f"_xsrf={xsrf}"
        print(f"Connecting to {ws_url}...")
        async with websockets.connect(ws_url, additional_headers=ws_headers, open_timeout=30.0) as ws:
            print("Connected!")
